...
1. **Frontend**
  ...
  - React is used as the core library for building UI components.

  1.2 **State Management Options**
  Available strategies for managing client state:

  - **Context API** – Built into React, good for simple state sharing, but scales poorly with complex applications.
  - **SWR** – Lightweight library focused on data fetching and caching; lacks full global state control.
  - **React Query** – Excellent for server state management, caching, and synchronization, but limited for broader application state.
  - **Redux Toolkit (RTK)** – Centralized, predictable state container with rich ecosystem, DevTools, and middleware support.
  ...
  - **Team Scalability**  
    Standardized patterns, DevTools integration, and mature ecosystem simplify onboarding and scaling.
  - **Flexibility & Control**  
  ...
  - **RTK Query Advantage**  
    Integrates React Query–like features *inside Redux*, providing advanced server-state management without adding another library.
